...
const mediumQuestionsScene = new BaseScene('mediumQuestions')

mediumQuestionsScene.enter(async ctx => {
	try {
...
mediumQuestionsScene.action(CORRECT_OPTION_CHOSEN_EVENT, async ctx => {
	try {
		await ctx.replyWithHTML(
			`THIS IS THE CORRECT ANSWER &#9989;${os.EOL}GOOD JOB!`
		)
		await ctx.scene.enter('mediumQuestions')
		ctx.answerCbQuery()
	} catch (error) {
...
mediumQuestionsScene.action(INCORRECT_OPTION_CHOSEN_EVENT, async ctx => {
	try {
		ctx.replyWithHTML(
...
mediumQuestionsScene.hears('Stop', async ctx => await ctx.scene.enter('welcome'))

module.exports = mediumQuestionsScene
