...
const easyQuestionsScene = new BaseScene('easyQuestions')

easyQuestionsScene.enter(async ctx => {
	try {
...
easyQuestionsScene.action(CORRECT_OPTION_CHOSEN_EVENT, async ctx => {
	try {
		await ctx.replyWithHTML(
			`THIS IS THE CORRECT ANSWER &#9989;${os.EOL}GOOD JOB!`
		)
		await ctx.scene.enter('easyQuestions')
		ctx.answerCbQuery()
	} catch (error) {
...
easyQuestionsScene.action(INCORRECT_OPTION_CHOSEN_EVENT, async ctx => {
	try {
		ctx.replyWithHTML(
...
easyQuestionsScene.hears('Stop', async ctx => await ctx.scene.enter('welcome'))

module.exports = easyQuestionsScene
